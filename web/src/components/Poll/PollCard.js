      ...
      <CardBody>
        <Row className="media align-items-center mb-4">
          <Link to={'/profile/' + poll.author?.userUrlId}>
            <img
              className="avatar-xs img-thumbnail rounded-circle mr-2"
          ...
          <div className="media-body align-items-center">
            <Link
              to={'/profile/' + poll.author?.userUrlId}
              className="text-muted font-size-10 mb-0"
            >
              {poll.author?.name}
            </Link>
            <p className="text-muted font-size-10 mb-0">
              {getRelativeTime(poll.modifiedDate)}
            </p>
          </div>
          {authUser && authUser._id === poll.author?._id && (
            <UncontrolledDropdown className="float-right mr-1">
              ...
              <DropdownMenu right>
                {editPollHandler && (
                  <DropdownItem onClick={editPollHandler}>Edit</DropdownItem>
                )}
          ...
          <div className="media-body">
            ...
            <div>
              {poll?.tags?.map((tag, index) => (
                <Link
                  to="#"
                  className="badge badge-light font-size-11 mr-1"
                  key={index}
                >
                  {tag}
                </Link>
